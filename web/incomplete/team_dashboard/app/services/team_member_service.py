"""
Team Member service layer for business logic operations.

This module contains the business logic for team membership operations,
including adding, removing, and updating team members with proper
...
def add_member(db: Session, team_id: int, creator_id: int, new_member_data: TeamMemberCreate) -> TeamMember:
    ...
    Validates permissions, user existence, and prevents duplicate memberships.
    ...
    Returns:
        TeamMember: Newly created team member object
        
    Raises:
        HTTPException: 404 if team or user not found
        HTTPException: 403 if creator is not a moderator
...
def remove_member(db: Session, team_id: int, remover_id: int, user_id: int) -> bool:
    ...
    Validates moderator permissions and ensures the target member exists in the team.
    ...
    Raises:
        HTTPException: 403 if remover is not a moderator
...
def update_member_role(db: Session, team_id: int, actor_id: int, user_id: int, update_data: TeamMemberUpdate) -> TeamMember:
    ...
    Allows moderators to promote or demote other members' moderator status.
    ...
    Returns:
        TeamMember: Updated team member object
        
    Raises:
        HTTPException: 403 if actor is not a moderator
...
def get_team_members(db: Session, team_id: int, requester_id: int) -> list[TeamMember]:
    ...
    Implements access control - only team members can view the team's member list.
    
    Args:
        db: Database session
        team_id: ID of the team
        requester_id: ID of the user requesting the member list
...
def get_member_by_id(db: Session, member_id: int) -> TeamMember:
    ...
    Returns:
        TeamMember: Requested team member object
